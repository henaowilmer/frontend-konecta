...
import tableData from "layouts/requests/data-table";
import employeeData from "layouts/requests/data-employee";
...
const FormRequest = lazy(() => import("layouts/requests/form"));
...
function Tables() {
  ...
  async function fetchEmployees() {
    try {
      ...
      setDataEmployees(result.employees);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }
  ...
  const handleClickOpenEdit = (index) => {
    if(authContext.isAdmin){
      const requestId = data.rows[index].requestId;
  ...
  const handleDeleteRequest = async () => {
    try {
      const requestId = data.rows[selectedRequestIndex].requestId;
  ...
  const renderSuccessSB = (
    <MDSnackbar
        ...
        icon="check"
        title="Éxito"
        content="Éxito, Solicitud eliminado correctamente"
        dateTime=""
        open={successSB}
    ...
    <DashboardNavbar />
      ...
      <Suspense fallback={<CircularProgress color="info" />}>
        ...
        <Dialog
          ...
          disableScrollLock
        >
          <FormRequest onFinish={handleFormFinish} updateId={updateId} employeesOptions={employees}/>
        </Dialog>
      </Suspense>
      {renderSuccessSB}
      {renderErrorSB}
