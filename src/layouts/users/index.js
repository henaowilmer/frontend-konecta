...
import tableData from "layouts/users/data";
import Pagination from "@mui/material/Pagination";
import Button from "@mui/material/Button";
import MDSnackbar from "components/MDSnackbar";
import { AuthContext } from "context";
const DataTable = lazy(() => import("examples/Tables/DataTable"));
const FormUser = lazy(() => import("layouts/users/form"));
...
function Tables() {
  ...
  const handleClickOpenEdit = (index) => {
    if(authContext.isAdmin){
      const userId = data.rows[index].userId;
      setUpdateId(userId);
  ...
  const handleDeleteUser = async () => {
    try {
      const userId = data.rows[selectedUserIndex].userId;
  ...
  const renderSuccessSB = (
    <MDSnackbar
        ...
        icon="check"
        title="Éxito"
        content="Éxito, Usuario eliminado correctamente"
        dateTime=""
        open={successSB}
    ...
    <DashboardNavbar />
      ...
      <Suspense fallback={<CircularProgress color="info" />}>
        ...
        <Dialog
          ...
          disableScrollLock
        >
          <FormUser onFinish={handleFormFinish} updateId={updateId} />
        </Dialog>
      </Suspense>
      {renderSuccessSB}
      {renderErrorSB}
